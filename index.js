...
pool.connect((err, client, done)=>{
    if(err){
     console.log("Database ERROR")
      ...
      console.log("Database conected3")
    ...
    client.on('notification', (msg) =>{
      ...
      let agresor,orden_id;
      let nombre_agresor,nombre_victima,victima,distanciaProhibida;
      let UltimaPosVictima, UltimaHoraFechaVictima,latVictima,latAgresor, lonAgresor, lonVictima;
  
      
     // Envio de alerta a EMAIL
     pool
    .query('SELECT * FROM orden where id_victima = $1',[ obj['id_victima'] ])
    .then((res) => {
      agresor=res.rows[0]['id_agresor']
      orden_id=res.rows[0]['id_orden']         
      distanciaProhibida=res.rows[0]['distancia_prohibida']
      ...
      pool
      .query("select * from victima where id_victima = $1",[ res.rows[0]['id_victima']  ])
      .then((res) => {
        victima=res.rows[0]['id_victima']
        ...
        .then((res) => {
          // inicia el envio de datos via EMAIL
          UltimaHoraFechaVictima=`${res.rows[0]['hora']||"0"},${res.rows[0]['fecha']||"0"}` 
          UltimaPosVictima=`${res.rows[0]['longitud']||"0"},${res.rows[0]['latitud']||"0"}`     
          latVictima=`${res.rows[0]['latitud']||"0"}`
          lonVictima=`${res.rows[0]['longitud']||"0"}`
          pool
          .query("select * from agresor where id_agresor = $1",[ agresor ])
          .then((res) => {
            nombre_agresor= `${res.rows[0]['p_nombre']} ${res.rows[0]['s_nombre']||" "} ${res.rows[0]['p_apellido']} ${res.rows[0]['s_apellido']||" "}`
            pool
            .query("select * from ubicacion_agresor where id_agresor = $1 order by id_ubicacion_agresor DESC LIMIT 1;",[ agresor ])
            .then((res) => {
              ...
              const mailOptions = {
                ...
                content: `{"numero de orden":"${orden_id}",
                        ...
                        "Victima":{
                          "Nombre Completo":"${nombre_victima}",
                          "Ultima Posicion Reciente":"${UltimaPosVictima}",
                          "Ultima Hora y Fecha Reciente":"${UltimaHoraFechaVictima}"
                        }
                        }`
                }]
                };   
    ...
    //Fin Agregamos posicion agresor notification
      ...
      const query2 = client.query("LISTEN update_posicion_victima")
    }
    });
...
app.get('/estadobanda', async (request, response) => {
  const id_agresor = request.query.id_agresor
  ...
  pool
    ...
    .then((res) => {
      ...
      id_orden= res.rows[0]['id_orden']
      pool.query("update  control_alertas set estado = $1 where id_orden = $2 and id_agresor = $3",[  estado, id_orden, id_agresor  ])
